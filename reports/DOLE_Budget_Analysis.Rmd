...
agency_meta <- tibble::tribble(
  ...
  "Office of the Secretary",                         "OSEC",  "osec",     2027,
  "Institute for Labor Studies",                     "ILS",   "current",  2027,
  "National Conciliation and Mediation Board",       "NCMB",  "current",  2027,
  "National Labor Relations Commission",             "NLRC",  "current",  2027,
  "National Wages and Productivity Commission",      "NWPC",  "current",  2027,
  "Professional Regulation Commission",              "PRC",   "current",  2027,
...
agency_evo_plot <- function(sh, subtitle = "NEP (proposed) vs GAA (enacted)") {
  ...
  ggplot(d, aes(year, total, color = metric, group = metric)) +
    ...
    scale_x_continuous(breaks = unique(c(seq(2018, m$last_year, by = 2), m$last_year))) +
...
agency_top_paps <- function(sh, n = 7) {
  m <- ag_meta(sh)
  base <- df_long %>% filter(agency == m$tab_name, year == m$last_year, !is.na(amount), amount > 0)
  # For FY 2027 there is no enacted GAA yet, so fall back to the NEP (proposed).
  use_metric <- if (any(base$metric == "GAA")) "GAA" else "NEP"
  basis_lab  <- if (use_metric == "GAA") "GAA" else "NEP, proposed"
  d <- base %>% filter(metric == use_metric) %>%
  ...
  ggplot(d, aes(pap, amount)) + geom_col(fill = if (use_metric == "GAA") bar_fill else "#54278F", width = 0.72) +
    ...
    labs(title = paste0(m$short, ": largest P/A/Ps (FY ", m$last_year, " ", basis_lab, ")"), x = NULL, y = NULL) +
...
## The department's largest P/A/Ps (FY 2027 NEP, proposed)
...
### Every P/A/P: FY 2026 enacted and FY 2027 proposed (interactive) {.unnumbered}
